import type { Work } from "../types/types.ts";

export const works: Work[] = [
  {
    id: "tensura",
    title: "That Time I Got Reincarnated as a Slime",
    description: `
# Reencarnado como um Slime
Depois de ser morto por um ladrão que fugia, um rapaz normal de **37 anos de idade** se encontra reencarnado em um outro mundo como um **slime cego com habilidades únicas**.

Com um novo nome — **Rimuru Tempest** — ele chega depois de conhecer seu novo amigo, o **"nível de catástrofe", Dragão da Tempestade Verudora**.
A partir daí, começa sua vida como slime em outro mundo, agora cercado por um **crescente número de seguidores**.

> Uma nova vida. Um novo corpo. Um poder além da imaginação.
`,
    image: "https://assets.pikiran-rakyat.com/crop/0x0:0x0/720x0/webp/photo/2024/08/28/666423672.jpg",
    characterId: [],
  },
  {
    id: "kagenojitsuryokusha",
    title: "Kage no Jitsuryokusha ni Naritakute!",
    description: `
# The Eminence in Shadow
Cid Kagenou sempre sonhou em se tornar uma figura poderosa nas sombras, manipulando tudo por trás dos bastidores.

Depois de morrer em seu mundo original, ele reencarna em um mundo de fantasia onde finalmente pode colocar seus planos em prática — criando uma organização fictícia chamada **Shadow Garden** para combater um culto maligno... que misteriosamente **existe de verdade**.

> Ele só queria brincar de herói nas sombras… e acabou se tornando uma lenda.
`,
    image: "https://media.graphassets.com/output=format:webp/LNWdajKJRSOPX452Nro9",
    characterId: [],
  },
  {
    id: "sousounofrieren",
    title: "Sousou no Frieren",
    description: `
# Frieren e o Pós-Fim da Jornada
Após derrotar o Rei Demônio, o grupo de heróis se desfaz, e a elfa maga **Frieren**, que tem uma vida muito mais longa que seus companheiros, segue seu caminho sozinha.

Com o passar dos anos, ela percebe o peso da passagem do tempo e decide **entender melhor os humanos**, embarcando em uma nova jornada repleta de memórias, arrependimentos e laços inesperados.

> Uma história não sobre salvar o mundo, mas sobre lidar com o tempo e o que ele deixa para trás.
`,
    image: "https://m.media-amazon.com/images/S/pv-target-images/b054f79f938837f13e1be59cb3d0cf4afcc4f4923ceae6fc192837b03fa726eb.jpg",
    characterId: [],
  },
];
